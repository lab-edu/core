...
public interface AssignmentSubmissionRepository extends JpaRepository<AssignmentSubmission, UUID> {
    ...
    @Modifying
    @Query("update AssignmentSubmission s set s.latest = false where s.assignment.id = :assignmentId and s.submittedBy.id = :userId and s.latest = true")
    int markLatestFalseByAssignmentIdAndUserId(@Param("assignmentId") UUID assignmentId, @Param("userId") UUID userId);

    @Query("select s from AssignmentSubmission s join fetch s.submittedBy join fetch s.assignment a join fetch a.course left join fetch s.gradedBy where s.id = :submissionId")
    Optional<AssignmentSubmission> findWithRelationsById(@Param("submissionId") UUID submissionId);
}